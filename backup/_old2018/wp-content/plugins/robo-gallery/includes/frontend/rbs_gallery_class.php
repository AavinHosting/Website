					...
					switch(get_post_meta( $this->id, ROBO_GALLERY_PREFIX.'lightboxDescSource', true )){
						...
						case 'desc': 
							...
							if(!empty($link)) { 
								...
								$site_url= get_permalink($Imageid);
							//	$site_url= '#';
							}
							...
							if($datalenth=='200') 
								$show_button=true;
							if($show_button) {
								$descBoxData .= '<form action="'.$site_url.'" method="post">';
								$descBoxData .= '<input type="hidden" name="galleryid" value="'.$Imageid.'">';
								$descBoxData .= '<input type="submit" name="submit" class="viewmore" value="View More">';
								$descBoxData .= '</form>';
							}
							break;
